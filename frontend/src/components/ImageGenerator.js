...
const ImageGenerator = () => {
  ...
  const handleDownloadAsset = () => {
    ...
    link.href = result.assetType === 'image'
      ? getResolvedImageUrl(result.assetUrl)
      : getResolvedAssetUrl(result.assetUrl);
    link.download = `${result.generationMode}-${Date.now()}.${getFileExtension(result.assetUrl, result.assetType)}`;
    link.click();
  };
  ...
  const handlePublishToExplore = async () => {
    ...
    try {
      ...
      setPublishMessage('Image published to Explore!');
    } catch (err) {
