...
const normalizeComment = (comment = {}) => ({
  _id: String(comment._id || generateId()),
  userId: normalizeId(comment.userId),
  username: String(comment.username || '').trim(),
  userAvatarUrl: String(comment.userAvatarUrl || '').trim(),
...
const normalizeGalleryPost = (post = {}) => ({
  ...
  comments: Array.isArray(post.comments)
    ? post.comments.map(normalizeComment).filter((comment) => comment.userId && comment.content)
    : [],
});
...
const loadStore = () => {
  try {
    ...
    state.galleryPosts = Array.isArray(parsedStore.galleryPosts)
      ? parsedStore.galleryPosts.map((post) => normalizeGalleryPost(post))
      : [];
  } catch (error) {
...
const syncGalleryPostsForAuthorFollowers = (userId, authorFollowersCount = 0) => {
...
const createUser = async ({
  ...
  const duplicateUser = state.users.find(
    (user) =>
      ...
      (normalizedReferralCode && user.referralCode === normalizedReferralCode)
  );
  ...
  const user = {
    ...
    followers: [],
    following: [],
    tokenBalance: Number.isFinite(Number(tokenBalance)) ? Number(tokenBalance) : DEFAULT_STARTING_TOKENS,
    referralCode: normalizedReferralCode,
    referredByUserId: normalizeId(referredByUserId),
    rewardedLikePostIds: [],
...
const createDemoUser = async () => {
  const existingDemoUser = state.users.find(user => user.email === 'demo@example.com');
  if (!existingDemoUser) {
    try {
      await createUser({
        username: 'demo',
...
const createGalleryPost = async ({
  ...
  const normalizedGalleryPost = normalizeGalleryPost(galleryPost);
  state.galleryPosts.push(normalizedGalleryPost);
  saveStore();
  return normalizedGalleryPost;
};
...
module.exports = {
  ...
  persistStore,
  sanitizeUser,
  syncGalleryPostsForAuthorFollowers,
  toggleFollowUser,
